...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  const flag = freshUser.changedPasswordAfter(decoded.iat);
  // console.log(flag);
...
exports.restrictTo = (...roles) => {
  ...
  return (req, res, next) => {
    ...
    if (!roles.includes(req.user.role)) {
      return next(
        new AppError(
          `This role doesn't have permission to do this action`,
          403,
        ),
      );
    }
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  user.passwordResetExpires = undefined;
  await user.save();
  // 3) Update cahngedPasswordAt property for the user
